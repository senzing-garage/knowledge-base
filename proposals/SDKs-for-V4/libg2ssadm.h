...
extern "C"
{
  ...
  _DLEXPORT long long G2SecureStoreAdmin_init(
                            const char *moduleName,
                            const char *iniParams,
                            const long long verboseLogging);
  _DLEXPORT long long G2SecureStoreAdmin_destroy();

  _DLEXPORT long long G2SecureStoreAdmin_initializeNewToken(
                            const char* defaultSOPin,
                            const char* newSOPin,
                            const char* label);
  _DLEXPORT long long G2SecureStoreAdmin_reinitializeToken(
                            const char* soPin,
                            const char* label);

  _DLEXPORT long long G2SecureStoreAdmin_setupStore(const char* soPin);

  _DLEXPORT long long G2SecureStoreAdmin_list(
                            char **responseBuf,
                            size_t *bufSize,
                            void *(*resizeFunc)(void *ptr, size_t newSize) );
  _DLEXPORT long long G2SecureStoreAdmin_put(
                            const char* soPin,
                            const char* label,
                            const char* value);

  _DLEXPORT long long G2SecureStoreAdmin_createSaltInStore(
                            const char* soPin,
                            const char* name,
                            const char* method,
                            char *buffer,
